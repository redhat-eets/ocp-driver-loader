...
podman pull --authfile ${OPENSHIFT_SECRET_FILE} ${DRIVER_TOOLKIT_IMAGE} >/dev/null 2>/dev/null

podman rm -f driver-toolkit >/dev/null 2>/dev/null || true
podman run -it -d --entrypoint /bin/bash --name driver-toolkit ${DRIVER_TOOLKIT_IMAGE} >/dev/null 2>/dev/null
...
podman rm driver-toolkit -f >/dev/null 2>/dev/null
